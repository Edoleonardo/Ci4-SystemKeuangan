            ...
            <table>
              <tr>
                <td style="font-size:10px; background-color: <?= $wrn ?> !important; -webkit-print-color-adjust: exact; text-align: center; max-width: 32mm;min-width: 32mm">
                  <b><?= $row['kode'] ?></b> <b style="color: red;"><?= $row['kadar'] ?></b> <?= (substr($row['kode'], 0, 1) == 1) ? '<b style="color: red;">- ' . $row['nilai_tukar'] . '%</b>' : '' ?> <br> <?= $row['jenis'] ?>, <?= $row['model'] ?><br>NW <?= $row['berat'] ?>Gr / GW <?= $row['berat'] + 0.12 ?>Gr <?php if (substr($row['kode'], 0, 1) == '1' && $row['kadar'] == '24K') {
