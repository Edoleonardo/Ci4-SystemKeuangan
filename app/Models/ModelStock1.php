...
class ModelStock1 extends Model
{

    protected $table = 'tbl_stock_1';
    protected $primaryKey = 'id_stock_1';
    ...
    public function getBarang($id = false)
    {
        if ($id == false) {
            $db = db_connect();
            $data = $db->query('select * from tbl_stock_1 order by created_at DESC limit 10');
    ...
    public function getBarangFilter($tampil, $stock, $kode)
    {
        ...
        $data = $db->query('select * from tbl_stock_1 where ' . $qty . ' and ' . $search . ' order by created_at DESC ' . $row . ' ');
        return $data->getResult('array');
    }
    ...
    {
        $db = db_connect();
        $data = $db->query('select * from tbl_stock_1 where substr(barcode,1,1) = ' . $id . ' order by created_at DESC');
        return $data->getResult('array');
    }
    public function CheckData($id)
    {
        $db = db_connect();
        $data = $db->query('select * from tbl_stock_1 where barcode = ' . $id . ' and substr(barcode,1,1) = 4  and substr(barcode,1,1) = 3');
        if ($data->getResult('array')) {
    ...
    {
        $db = db_connect();
        $data = $db->query('select * from tbl_stock_1 where substr(barcode,1,1) = 4 ');
        return $data->getResult('array');
        //  $this->get();
        // return $query;
    }
