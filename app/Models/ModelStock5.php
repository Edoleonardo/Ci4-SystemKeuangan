...
class ModelStock5 extends Model
{
    ...
    public function getBarang($id = false)
    {
        if ($id == false) {
            $db = db_connect();
            $data = $db->query('select * from tbl_stock_5 order by created_at DESC limit 10');
            return $data->getResult('array');
        }
        return $this->where(['id_stock_5' => $id])->first();
    }

    public function getBarangQty()
    {
        $db = db_connect();
        $data = $db->query('select * from tbl_stock_5 where qty != 0');
        return $data->getResult('array');
    }
    ...
    public function getBarangFilter($tampil, $stock, $kode)
    {
        ...
        $data = $db->query('select * from tbl_stock_5 where ' . $qty . ' and ' . $search . ' order by created_at DESC ' . $row . ' ');
        return $data->getResult('array');
    }
    ...
    {
        $db = db_connect();
        $data = $db->query('select * from tbl_stock_5 where barcode = ' . $id . ' and substr(barcode,1,1) = 4  and substr(barcode,1,1) = 3');
        if ($data->getResult('array')) {
    ...
    {
        $db = db_connect();
        $data = $db->query('select * from tbl_stock_5 where barcode = ' . $id . ' ');
        if ($data->getResult('array')) {
            $data = $data->getResult('array')[0];
        } else {
            $data = $data->getResult('array');
    ...
    {
        $db = db_connect();
        $data = $db->query('select max(substr(barcode,2,7)) kode from tbl_stock_5 where substr(barcode,1,1) = ' . $id . ' limit 1');
        return $data->getResult('array')[0];
        //  $this->get();
        // return $query;
    }
    ...
    {
        $db = db_connect();
        $data = $db->query('select * from tbl_stock_5 where substr(barcode,1,1) = 4 ');
        return $data->getResult('array');
        //  $this->get();
        // return $query;
    }
    ...
    {
        $db = db_connect();
        $data = $db->query('DELETE FROM tbl_stock_5 WHERE barcode = ' . $id . '');
        return 0;
        //  $this->get();
        // return $query;
    }
    ...
    {
        $db = db_connect();
        $data = $db->query('SELECT sum(qty) as qty FROM `tbl_stock_5` WHERE qty != 0 AND kadar = ' . '"' . $kdr . '"' . ' AND jenis = ' . '"' . $jns . '"' . ';');
        return $data->getResult('array');
    }
    public function SumDataHomeBrt($kdr, $jns)
    {
        $db = db_connect();
        $data = $db->query('SELECT sum(berat) as berat FROM `tbl_stock_5` WHERE qty != 0 AND kadar = ' . '"' . $kdr . '"' . ' AND jenis = ' . '"' . $jns . '"' . ';');
        return $data->getResult('array');
    }
    public function SumDataTotalQty($kdr)
    {
        $db = db_connect();
        $data = $db->query('SELECT sum(qty) as qty FROM `tbl_stock_5` WHERE qty != 0 AND kadar = ' . '"' . $kdr . '"' . ';');
        return $data->getResult('array');
    }
    public function SumDataTotalBrt($kdr)
    {
        $db = db_connect();
        $data = $db->query('SELECT sum(berat) as berat FROM `tbl_stock_5` WHERE qty != 0 AND kadar = ' . '"' . $kdr . '"' . ';');
        return $data->getResult('array');
    }
    public function SumDataGrandTotalQty()
    {
        $db = db_connect();
        $data = $db->query('SELECT sum(qty) as qty FROM `tbl_stock_5` WHERE qty != 0;');
        return $data->getResult('array');
    }
    public function SumDataGrandTotalBrt()
    {
        $db = db_connect();
        $data = $db->query('SELECT sum(berat) as berat FROM `tbl_stock_5` WHERE qty != 0;');
        return $data->getResult('array');
    }
}
