...
class BuybackCust extends BaseController
{
        ...
        if ($this->request->isAJAX()) {
            ...
            if ($databuyback) {
                $this->modelbuyback->save([
                    ...
                    'id_karyawan' => $session->get('id_user'),
                    'kelompok' => ($totaljumlah['total'] == null) ? 0 : $databuyback['kelompok'],
            ...
            if (!$valid) {
                ...
                if ($datacust) {
                    ...
                    if ($saldobiaya['total_akhir_tunai'] >= $this->request->getVar('tunai') && !$this->request->getVar('transfer')) {
                        $sukses = true;
                    } elseif ($this->request->getVar('transfer') && !$this->request->getVar('tunai')) {
                        $sukses = true;
                    } elseif ($saldobiaya['total_akhir_tunai'] >= $this->request->getVar('tunai') && $this->request->getVar('transfer')) {
                        $sukses = true;
                    } else {
                        $sukses = false;
                    }
                    if ($this->request->getVar('pembulatan') != $databuyback['total_harga']) {
                        if ($sukses) {
    ...
    public function TambahBuybackNonota()
    {
        if ($this->request->isAJAX()) {
            ...
            if ($this->request->getVar('kelompok') == 1 || $this->request->getVar('kelompok') == 2) {
            ...
            if (!$valid) {
                $msg = [
                    'error' => [
                        ...
                        'kel' => $this->request->getVar('kelompok')
                    ]
                ];
                echo json_encode($msg);
            } else {
                ...
                if ($statuskode) {
                    ...
                    if ($databuyback['kelompok'] == $this->request->getVar('kelompok') || $databuyback['kelompok'] == 0) {
                        if ($filesampul->getError() != 4 || $this->request->getPost('gambar')) {
                            if (isset($checkkode)) {
                                if ($checkkode['nama_img'] != 'default.jpg') { //buyback dengan nota, foto ikut terhapus
                                    if ($checkkode['nama_img'] != 'default.jpg') {
                                        if (file_exists('img/' . $checkkode['nama_img'])) {
                                            unlink('img/' . $checkkode['nama_img']); //untuk hapus file
                                        }
                                    }
                                }
                            }
                            if ($this->request->getPost('gambar')) {
                                ...
                                $namafile = date('ymdhis') . $barcode . '.jpg';
                                ...
                                if ($filesampul->getError() == 4) {
                                    ...
                                    $namafile = date('ymdhis') . $barcode . '.jpg'; // pake nama random
                        ...
                        if ($kode == 1) {
                            ...
                            if (!$this->request->getVar('barcode')) {
                                $this->datastock3->save([
                                    ...
                                    'harga_beli' =>  $this->request->getVar('harga_beli'),
                                    'total_harga' => $totalharga,
                                    'gambar' => ($oldimg) ? $checkkode['gambar'] : $namafile,
                                ]);
                            }
                        } elseif ($kode == 4) {
                            $totalharga =  $berat *  $harga;
                        ...
                        $this->modeldetailbuyback->save([
                            'nama_img' => ($oldimg) ? $checkkode['gambar'] : $namafile,
                            'id_karyawan' => $session->get('id_user'),
                ...
                if (isset($datakartu)) {
                    ...
                    if ($data['nama_img'] != 'default.jpg') { //buyback dengan nota, foto ikut terhapus
                        if ($data['nama_img'] != 'default.jpg') {
                            if (file_exists('img/' . $data['nama_img'])) {
